$().ready(function() {
	...
	var offline;
	$('#DesGeneralOptionTable').on('draw.dt', function() {
		resetDtWidth();
	}).DataTable({
		"serverSide": true,
		"ajax": {
			"url": CTX + "/bt/option/listvo",
			"type": "post",
			"data": {
                "perms[0]": "stem_update",
                "perms[1]": "stem_add",
	...
	/**当前选中 高亮显示*/
	$("#DesGeneralOptionTable").on("click", "tr", function() {
		$("#DesGeneralOptionTable tbody tr.highlight").each(function(){
			console.log($(this));
			if($(this)!=null){
	...
	$("#searchBtn").on('click', function() {
		$('#DesGeneralOptionTable').DataTable().column(0).search($("#username").val(), true).column(1).search($("#realName").val(), true).draw();
	});

	/** 添加 */
	$("#addBtn").on("click", function() {
		$("#modal").load(CTX + "/bt/option/addView");
		$("#modal").modal("show");
	});
	...
	/** 禁用 */
	$("#DesGeneralOptionTable").on("click", ".dataTables_toolcolumn .disable", function() {
			...
			if (isConfirm) {
				$.ajax({
					...
					success: function(message) {
						...
						$('#DesGeneralOptionTable').DataTable().draw(false);
					},
	...
	/** 编辑 */
	$("#DesGeneralOptionTable").on("click", ".dataTables_toolcolumn .edit", function() {
		var id = $(this).parents("tr").attr("id");
		$("#modal").load(CTX + "/bt/option/editView",{"id":id});
		$("#modal").modal("show");
	});
});